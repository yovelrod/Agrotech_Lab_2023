...
WindDirection directions[] = {
  {1, "North Wind", 0.0, 0.7143},
  {2, "Northeasterly Wind", 0.7143, 1.4286},
  {3, "East Wind", 1.4286, 2.1429},
  {4, "Southeast Wind", 2.1429, 2.8571},
  {5, "South Wind", 2.8571, 3.5714},
  {6, "Southwest Wind", 3.5714, 4.2857},
  {7, "West Wind", 4.2857, 5.0},
  {8, "Northwest Wind", 0.0, 5.0}
};
  ...
  if ((millis() - lastDirectionSampleTime >= SAMPLE_INTERVAL)) {
    for (int i = 0; i < sizeof(directions) / sizeof(directions[0]); i++) {
      if (voltage >= directions[i].voltageMin && voltage < directions[i].voltageMax) {
        ...
        Serial.println(getWindDirectionString(directions[i].number));
        Serial.println("\n\n*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*");
